...
// 时间格式化函数 - 转换为航空报文格式
function formatAviationTime(isoString) {
...
// 时间范围格式化
function formatTimeRange(fromUnix, toUnix) {
    if (!fromUnix || !toUnix) return '未知时间范围';
    
    try {
        const fromDate = new Date(fromUnix * 1000);
        const toDate = new Date(toUnix * 1000);
        
        const fromStr = `${String(fromDate.getUTCDate()).padStart(2, '0')}${String(fromDate.getUTCHours()).padStart(2, '0')}${String(fromDate.getUTCMinutes()).padStart(2, '0')}`;
        const toStr = `${String(toDate.getUTCDate()).padStart(2, '0')}${String(toDate.getUTCHours()).padStart(2, '0')}${String(toDate.getUTCMinutes()).padStart(2, '0')}`;
...
// 英里转公里
function convertVisibility(visibilityStr) {
    if (!visibilityStr) return '未知';
    
    if (visibilityStr === '6+') {
        return '大于10公里';
    } else if (visibilityStr === '10+') {
        return '大于16公里';
    } else if (visibilityStr.includes('英里')) {
        const miles = parseFloat(visibilityStr);
        if (!isNaN(miles)) {
...
function updateMetarDisplay(data) {
    ...
    // 更新各个字段
    document.getElementById('metar-observation-time').textContent = formatAviationTime(data.basicInfo?.observationTime);
    ...
    document.getElementById('metar-visibility').textContent = convertVisibility(data.visibility?.info);
...
function updateTafDisplay(data) {
    ...
    // 更新基本信息
    document.getElementById('taf-issue-time').textContent = formatAviationTime(data.basicInfo?.issueTime);
            ...
            <div class="forecast-period">
                ...
                <div class="period-info">
                    ...
                    <div class="info-row">
                        ...
                        <span class="value">${formatAviationTime(period.becomeTime)}</span>
                    </div>
                    ` : ''}
                    ${period.probability ? `
                    <div class="info-row">
                    ...
                    <div class="info-row">
                        ...
                        <span class="value">${convertVisibility(period.visibility.info)}</span>
                    </div>
                    <div class="info-row">
